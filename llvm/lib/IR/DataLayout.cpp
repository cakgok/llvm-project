...
DataLayout &DataLayout::operator=(const DataLayout &Other) {
  ...
  DefaultNullPtrKind = Other.DefaultNullPtrKind;
...
Error DataLayout::parseNullPointerDefaultSpec(StringRef Spec) {
  ...
  if (NullValue == 'z') {
    DefaultNullPtrKind = NullPtrKind::Zero;
  } else if (NullValue == 'o') {
    DefaultNullPtrKind = NullPtrKind::AllOnes;
  } else {
...
APInt DataLayout::getNullPtrValue(unsigned AS) const {
  ...
  if (DefaultNullPtrKind == NullPtrKind::Zero)
